...
?>
 <?php session_start();
  $conn = new mysqli('localhost', 'root', '', 'adminstrativetool');
  if(!$conn){
...
<body>
		...
		<label>DAMRO PLC </label>

			<?php
		 	$id = (int) $_SESSION['id'];

					$query = $conn->query ("SELECT * FROM customer WHERE cust_id = '$id' ") or die (mysqli_error());
					$fetch = $query->fetch_array ();
			?>

			<ul>
				<li><a href="function/logout.php"><i class="icon-off icon-white"></i>logout</a></li>
				<li>Welcome:&nbsp;&nbsp;&nbsp;<a href="#profile" href  data-toggle="modal"><i class="icon-user icon-white"></i><?php echo $fetch['cust_Name']; ?>&nbsp;<?php echo $fetch['email'];?></a></li>
			</ul>
	</div>
